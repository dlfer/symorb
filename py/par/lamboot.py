...
try:
    options, xarguments = getopt.getopt(
        sys.argv[1:], OptionsLetters, AllOptions)
except getopt.error:
    error_message()
    sys.exit(1)
...
def running_cpu(h):
    ...
    total_cpu = 0.0
    for l in result[1:]:
        total_cpu = total_cpu + str.atof(l)
    return total_cpu
...
def major_running_cpu(h):
    ...
    major_procs = ''
    for l in result[1:]:
        tmpvar = str.split(strip_spaces(l), ' ')[0]
        if str.atof(tmpvar) > 3:
...
def hostlist_write():
    ...
    for line in HOST_LIST:
        ...
        if totcpu < 110.0 or line == HOST_LIST[0]:
            fd_out.write("%s\n" % (line))
            sys.stderr.write(
                "adding host %16s    (%%CPU=%04.2f)\n" % (line, totcpu))
            numHosts += 1
        else:
            sys.stderr.write(
                "**skipped host %16s (%%CPU=%04.2f)\n" % (line, totcpu))
...
def sub_top():
    ...
    for line in HOST_LIST[1:]:
        ...
        stringa = str.split(minor_running_cpu(line), "\n")
        for item in stringa:
            tokens = str.split(strip_spaces(item), ' ')
            if len(tokens) > 1:
                try:
                    username = tokens[2]
                    cpuperc = str.atof(tokens[0])
                    command = tokens[1]
                    if username in list(user_dic.keys()):
                        if command in list((user_dic[username]).keys()):
                            user_dic[username][command] = user_dic[username][command] + \
                                cpuperc / float(len(HOST_LIST))
                        else:
                            user_dic[username][command] = cpuperc / \
                                float(len(HOST_LIST))
                    else:
                        user_dic[username] = {}
                        user_dic[username][command] = cpuperc / \
